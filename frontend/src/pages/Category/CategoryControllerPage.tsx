...
const CategoryControllerPage: React.FC<Props> = ({ type }) => {
  ...
  const getCategory = async () => {
    try {
      const res = await ForgeAxios({ method: "GET", url: `/category/${id}` });
  ...
  const isOwner = category ? category.isOwner : true;
    ...
    <div className="page">
      ...
      <Section sectionTitle="Alapadatok">
        <InputHolder text="Kategória neve">
          <input type="text" ref={nameRef} disabled={!canEdit} />
        </InputHolder>
        <InputHolder text="Kategória leírása">
          <textarea rows={5} ref={descriptionRef} disabled={!canEdit} />
        </InputHolder>
      </Section>
      ...
      {category && isOwner && (
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "var(--content-padding)" }}>
          <Button
            icon={<FaShare />}
            onClick={() => navigate(`/categories/${category.id}/share`)}
            style={{ width: "auto" }}
          >
            Megosztás kezelése
          </Button>
      ...
      {category && <Tasks categoryId={category.id} canEdit={canEdit} />}
    </div>
  );
};
